...
# and a time dependent price profile that varies sinusoidally.
...
function create_operational_objective(model, periods, price)
    spot = model[:spot]
    el_cost = sum(multiple(t) * price[t] * spot[t] for t in periods)
    return el_cost
end
...
optimize!(model)
println("Objective value: ", objective_value(model))
println("Battery sizing: ", value.(model[:cap]))

# ### RepresentativePeriods

# In large scale models, it is common to use representative periods to reduce the computational burden by aggregating
# the time series involved.
...
# Finnally, we can extend the model to include operational scenarios to account for operational uncertainty in
